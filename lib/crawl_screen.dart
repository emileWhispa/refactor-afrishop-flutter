                              ...
                              children: <Widget>[
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                      ...
                                      color: _selected?.skuStr == f.skuStr
                                          ? color
                                          : Colors.transparent,
                                      border: Border.all(
                                          width: 1,
                    ...
                    children: <Widget>[
                      ...
                      GestureDetector(
                        ...
                        onTap: () async {
                          setState(() {
                            _value +=
                                _value <= (_selected?.quantity ?? 0) ? 1 : 0;
                          });
                        },
                      )
                    ],
                      ...
                      onPressed: () {
                        if (_selected == null) {
                          platform.invokeMethod(
                              "toast", "Select descriptions above");
                          return;
                        }
...
//        builder: (context) => _Resolver(cart: _cart,list: _skuList,callback: widget.callback,user: widget.user,));
  ...
  void sendCart(count,img, str, price,itemSkuId) {
    var map = {
      "itemSku": str,
      "itemPrice": price,
      "itemImg": img,
      ...
      onValue: (source, url) {
        ...
        if (data != null) {
          ...
          sku.forEach((element) {
            ...
            double price = index < priceList.length
                ? double.tryParse(priceList[index]['price'])
                : 0.0;

            index++;

            skus.add(Sku(
                price, element['sellableQuantity'], element['skuStr'], sbs));
          });

          if (product.count <= 0 && skus.isNotEmpty) {
            product.count = skus.first.count;
          }
  ...
  Future<bool> canGoBack()async{
    ...
    var b = await _controller.canGoBack();
    if( b ){
      _controller.goBack();
    }
    return !b;
              ...
              children: <Widget>[
                ...
                Column(
                  ...
                  children: <Widget>[
                    ...
                    _cart == null
                        ? Text("\$0.0")
                        : Text("\$${_cart['itemPrice'] ?? 0.0}"),
                  ],
                ),
                Expanded(
                    child: Padding(
                  padding:
                      const EdgeInsets.only(left: 8.0, top: 6.0, bottom: 5.0),
                  child: _loading
                      ? Padding(
                      ...
                      : Container(
                          ...
                          child: RaisedButton(
                            ...
                            onPressed: product == null ? null : addToCart,
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                Text(
...
//    );
  ...
  TextStyle get _style => TextStyle(fontWeight: FontWeight.w900, fontSize: 18);
